...
// Helper to access lucaLinkManager dynamically
let lucaLinkManager: any = null;
if (typeof window !== "undefined") {
  import("./lucaLink/manager").then((m) => {
    lucaLinkManager = m.lucaLinkManager;
  });
}
...
class AppControlService {
  ...
  async openApp(appName: string): Promise<AppLaunchResult> {
    // 1. Web/Desktop Platform Logic
    if (!this.isMobile()) {
      // A. Try Desktop Bridge (via Cortex backend)
      const systemResult = await nativeControl.launchApp(appName);
      if (systemResult && systemResult.success) {
        return {
          success: true,
          message: `Opened ${appName} on your host machine.`,
        };
      }

      // B. Try Luca Link Delegation (Web user controlling connected phone)
      if (lucaLinkManager && lucaLinkManager.devices.size > 0) {
        const identifier = this.getAppIdentifier(appName);
        const device = Array.from(lucaLinkManager.devices.values()).find(
          (d: any) =>
            d.type === "android" || d.type === "mobile" || d.type === "ios",
        ) as any;

        if (device && identifier) {
          console.log(
            `[AppControl] Delegating openApp(${appName}) to mobile device: ${device.deviceId}`,
          );
          const res = await lucaLinkManager.delegateTool(
            device.deviceId,
            "controlMobileDevice",
            {
              action: "OPEN_APP",
          ...
          if (res.success) {
            return {
              success: true,
              message: `Delegated: Opened ${appName} on your mobile device.`,
            };
          }
        }
      }
      ...
      return {
        success: false,
        error: `App launching is not supported directly in the browser. Start the Cortex backend or connect a device via Luca Link.`,
      };
    }

    // 2. Native Mobile Logic
    const identifier = this.getAppIdentifier(appName);
