...
export class ComputerUseRuntimeEntrypoint {
  ...
  async runComputerUseStep(step: ComputerUseMissionStepInput): Promise<ComputerUseRuntimeEntrypointResult> {
    const validation = this.validateInput({ missionStepInput: step });
    if (!validation.ok) return this.invalidResult(validation.reason ?? "invalid mission step input");
  ...
  validateInput(input: ComputerUseRuntimeEntrypointInput): { ok: boolean; reason?: string } {
    if (input.missionStepInput) {
      if (input.missionStepInput.kind !== "computer_use") return { ok: false, reason: "mission step kind must be computer_use" };
      return { ok: Boolean(input.missionStepInput.missionId && input.missionStepInput.stepId) };
    }
