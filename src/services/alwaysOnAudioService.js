...
class AlwaysOnAudioService {
    constructor() {
        this.config = {
            ...
            instruction: null // User-defined instruction
        };
     ...
     */
    /**
     * Set user instruction for monitoring
     */
    setInstruction(text) {
        this.config.instruction = text;
        console.log(`[ALWAYS_ON_AUDIO] Custom instruction set: "${text}"`);
    }
    ...
    async analyzeAudio() {
        ...
        try {
            ...
            // Get model from settings (align with Vision/Multimodal settings)
            const sentryModel = settingsService.get("brain")?.visionModel || "gemini-2.0-flash";

            // Analyze with Gemini
            const analysisResult = await audioAnalyzerService.analyzeAudio(chunk.filePath, {
                customInstruction: this.config.instruction,
                model: sentryModel
            });
            ...
            if (analysisResult.success) {
                ...
                if (eventResult.success && eventResult.events.length > 0) {
                    ...
                    const filteredEvents = eventResult.events.filter(event =>
                        this.config.eventTypes.includes(event.type) || event.type === 'user_alert'
                    );
