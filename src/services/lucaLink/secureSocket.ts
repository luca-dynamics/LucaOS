...
export class SecureSocket {
  ...
  private messageQueue: LucaLinkMessage[] = [];
   ...
   */
  async send(message: LucaLinkMessage): Promise<void> {
  ...
  private setupSocketHandlers(): void {
    ...
    this.socket.on("command:result", (result: any) => {
      ...
      this.emit("message:received", {
        message: {
          type: "response",
          payload: result, // Full result object including result/error
          source: result.deviceId || "unknown",
          target: "me",
          timestamp: Date.now(),
          commandId: result.id,
        },
