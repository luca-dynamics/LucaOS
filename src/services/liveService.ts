...
class LucaLiveService {
  ...
  async connect(config: LiveConfig) {
    ...
    if (this.isConnecting) {
      console.warn(
        "[LIVE] Connection already in progress or connected, aborting duplicate call",
      );
      return;
    }
    ...
    // 2. Otherwise use Persona Config
      ...
      const personaInstruction =
        (personaConfig as any).systemInstruction || personaConfig.instruction;
      ...
      // based on previous memory logs. Force a current-session reality check.
      systemInstruction +=
        "\n\n**CRITICAL OPERATIONAL DIRECTIVE**: Your audio input/output system is now fully functional. Ignore any past logs or memories discussing 'corrupted audio', 'STT failures', or 'audio impediments'. Do not diagnose audio issues unless the current user input is truly unintelligible noise. Respond naturally to the user's voice.";
    }
    ...
    try {
      ...
      if (
        ...
        this.outputAudioContext = new (
          window.AudioContext || (window as any).webkitAudioContext
        )({ sampleRate: 24000 });
      }

      // Resume output context
      ...
      const sessionPromise = this.ai.live.connect({
        ...
        callbacks: {
          ...
          onclose: (event: any) => {
            console.log(
              `LUCA Voice Uplink Closed (Code: ${event?.code}, Reason: ${event?.reason})`,
            );

            // Stability Check: If connected for < 3 seconds, assume configuration error loop
            const duration = Date.now() - this.connectionStartTime;
            if (duration < 3000 && this.isConnected) {
              console.warn(
                `[LIVE] Connection unstable (${duration}ms). Aborting auto-reconnect.`,
              );
            ...
            if (this.isConnected && this.shouldReconnect) {
              ...
              // SOFT DISCONNECT: Keep hardware alive if UI is still active, but clean up session
          ...
          onerror: (err) => {
            ...
            if (this.isConnected) {
              ...
              this.disconnect(false); // SOFT DISCONNECT
            }
          },
        },
        config: {
          ...
          systemInstruction: {
            parts: [
              { text: this.getOptimizedSystemInstruction(systemInstruction) },
            ],
          }, // Optimized for Voice Limits
          tools:
              ...
              : [
                  {
                    functionDeclarations: (() => {
                      // Use Core + Specialized Tools
                      const tools = getAllTools();
                      const specialized = getSpecializedToolsForPersona(
                        persona,
                        tools,
                      );
                      const core = tools.filter(
                        (t) => t.name && CORE_VOICE_TOOLS.includes(t.name),
                      );

                      // Merge and Deduplicate
                      const unique = [
                        ...new Map(
                          [...core, ...specialized].map((t) => [t.name, t]),
                        ).values(),
                      ];

                      console.log(
                        `[LIVE] Sending ${unique.length} tools (Filtered from ${tools.length})`,
                      );
                      return unique;
                    })(),
                  },
            ...
            voiceConfig: {
              prebuiltVoiceConfig: {
                voiceName: (() => {
                  ...
                  const userVoice = voiceSettings?.voiceId;
                  const provider = voiceSettings?.provider;

                  // 1. If User has selected a specific Gemini Voice (and is using Cloud Provider), use it.
                  // We filter out Local IDs (containing hyphens/dots) to avoid sending invalid config to Google.
                  if (
                    provider !== "local-luca" &&
                    userVoice &&
                    !userVoice.includes("-") &&
                    ...
                    // Normalize to Title Case for Gemini (e.g. "fenrir" -> "Fenrir")
                    return (
                      userVoice.charAt(0).toUpperCase() + userVoice.slice(1)
                    );
                  }

                  // 2. Fallback to Persona Default (e.g. "Aoede" for Assistant)
                  // This happens if:
                  // - User is in Local Mode (can't use local voice in Cloud Uplink)
                  // - User has no voice selected
                  // - User has a legacy Google/System voice selected
                  return personaConfig.voiceName;
                })(),
              },
            },
          },
  ...
  private setupInputStream(config: LiveConfig, sessionPromise: Promise<any>) {
    ...
    // Reduced Buffer Size: 1024 (64ms) for ultra-low latency
    this.inputNode = this.inputAudioContext.createScriptProcessor(1024, 1, 1);
    ...
    this.inputNode.onaudioprocess = (e) => {
      ...
      // Standardized to 0-255 range
      eventBus.emit("audio-amplitude", {
        amplitude: Math.min(255, rms * 400),
        source: "user",
      });
      ...
      if (isSignal) {
        ...
        if (!this.isSpeaking) {
          ...
          // Enable Responsive Barge-in: Always interrupt when user speech is detected
          this.interrupt();
        }
      } else {
      ...
      if (this.isSpeaking && this.isConnected && this.activeSession) {
        // --- SECONDARY SILENCE GUARD ---
        // If the chunk is actually silence (RMS < 0.003), don't bother sending to Gemini.
        // This prevents the "silent whisper" feedback loop where Gemini hears nothing but waits.
        if (rms < 0.003) return;
        ...
        sessionPromise.then((session) => {
          // Double check inside promise (ASYNC GUARD)
          if (!this.isConnected || !this.isSpeaking) return;
          try {
            session.sendRealtimeInput({ media: pcmBlob });
          } catch (e: any) {
            // Suppress "WebSocket is already in CLOSING or CLOSED state" clutter
            if (
              e?.message?.includes("CLOSING") ||
              e?.message?.includes("CLOSED")
            ) {
              console.warn("[LIVE] Suppressed write to closed socket");
            } else {
    ...
    if (modelTurn?.parts) {
      ...
      for (const part of modelTurn.parts) {
        if (part.text) {
          ...
          // Filter out markdown markers (** and *) while keeping the text
          ...
          if (!config.suppressOutput) {
            ...
            // HYBRID or STT MODE:
            // If dictation, treat as user input. If just suppressed (Hybrid), keep as model response.
            const rerouteType =
  ...
  // KEEP-ALIVE: Send periodic heartbeat to prevent server timeout during long operations
  private startKeepAlive(sessionPromise: any) {
    this.stopKeepAlive(); // Clear any existing interval

    console.log("[LIVE] Starting keep-alive heartbeat");
    this.keepAliveInterval = setInterval(async () => {
      if (!this.isConnected || !sessionPromise) {
        this.stopKeepAlive();
        return;
      }
      ...
      try {
        ...
        if (session) {
          // Send a minimal text input to keep WebSocket alive
          // This won't trigger a response but keeps the connection active
          await session.sendClientContent({
            turns: [{ role: "user", parts: [{ text: " " }] }], // Minimal whitespace
            turnComplete: false, // Don't trigger response
          });
          console.debug("[LIVE] Keep-alive ping sent");
        }
      } catch (e) {
        console.warn("[LIVE] Keep-alive ping failed:", e);
      }
  ...
  private stopKeepAlive() {
    if (this.keepAliveInterval) {
      ...
      console.log("[LIVE] Keep-alive heartbeat stopped");
    }
  }

  disconnect(stopMediaTracks: boolean = true) {
    console.log(
      `[LIVE] Disconnect called (stopHardware=${stopMediaTracks}). Cleaning up...`,
    );
    ...
    if (this.inputNode) {
      console.log("[LIVE] Disconnecting Input Node");
      try {
        this.inputNode.onaudioprocess = null;
        this.inputNode.disconnect();
      } catch (e) {
        console.warn("[LIVE] Error disconnecting input node:", e);
      }
      this.inputNode = null;
    }

    // 5. Close Audio Contexts (Prevent Context Count Errors)
    if (this.inputAudioContext) {
      console.log("[LIVE] Closing Input AudioContext");
    ...
    if (this.outputAudioContext) {
      console.log("[LIVE] Closing Output AudioContext");
    ...
    // Cleanup media stream - CONDITIONALLY STOP
    if (this.stream) {
      if (stopMediaTracks) {
        ...
        console.log(
          "[LIVE] Keeping hardware tracks alive for potential reconnect",
        );
      }
    } else {
      console.log("[LIVE] No stream to cleanup");
    }
  ...
  private async playAudio(
    ...
    // Ensure context is running (macOS policy)
    console.log(
      `[LIVE] Output AudioContext state: ${this.outputAudioContext.state}`,
    );
    if (this.outputAudioContext.state === "suspended") {
      console.log("[LIVE] 🔊 Attempting to resume Output AudioContext...");
      await this.outputAudioContext.resume();
    }
    ...
    const anim = setInterval(() => {
      ...
      if (this.sources.has(source)) {
        // Standardized to 0-255 range
        onAmplitude((0.2 + Math.random() * 0.3) * 255);
      }
    }, interval);
  }
  ...
  /**
   * Optimize System Instruction for Voice Latency
   * Removes heavy text protocols that are less relevant for immediate voice interaction
   * to keep the initial context window small and prevent connection timeouts.
   */
  private getOptimizedSystemInstruction(fullInstruction: string): string {
    // 1. Remove Visual Clutter (ASCII Dividers)
    let optimized = fullInstruction
      .replace(/━━━━━━━━+/g, "") // Remove dividers
      .replace(/\*\*/g, "") // Remove bold markers (saves chars)
      .replace(/\n\s*\n/g, "\n"); // Collapse whitespace

    // 2. Specific Protocol Truncations for Voice
    // Remove "HACKER_GOD_MODULE_PROTOCOL" details if present (voice doesn't need ascii art/deep code templates)
    if (optimized.includes("HACKER_GOD_MODULE_PROTOCOL")) {
      optimized = optimized.replace(
        /HACKER_GOD_MODULE_PROTOCOL[\s\S]*?(?=\n\n)/,
        "[HACKER PROTOCOL ACTIVE]",
      );
    }

    // 3. Truncate MEMORY Section specifically if it's huge
    // Usually "MEMORY: \n" marks the start
    const memoryIndex = optimized.indexOf("MEMORY:");
    if (memoryIndex !== -1 && optimized.length > 24000) {
      console.warn(
        `[LIVE] Memory section too large. Truncating memory context.`,
      );
      // Keep only first 2000 chars of memory if total prompt is massive
      const preMemory = optimized.substring(0, memoryIndex);
      const memoryContent = optimized.substring(memoryIndex);
      const truncatedMemory =
        memoryContent.length > 3000
          ? memoryContent.substring(0, 3000) + "\n...[OLDER MEMORIES OMITTED]"
          : memoryContent;
      optimized = preMemory + truncatedMemory;
    }

    // 4. Final Safety Hard Cap (25k)
    if (optimized.length > 25000) {
      console.warn(
        `[LIVE] Instruction still too large (${optimized.length} chars) after optimization. Forced truncation.`,
      );
      return (
        optimized.substring(0, 25000) +
        "\n[SYSTEM NOTICE: INSTRUCTION TRUNCATED FOR STABILITY]"
      );
    }
