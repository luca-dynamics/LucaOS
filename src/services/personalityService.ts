...
export class PersonalityService {
  ...
  constructor() {
    ...
    // Sync with global settings (Theme/Persona)
    settingsService.on("settings-changed", (newSettings) => {
      const newTheme = newSettings.general?.theme;
      if (newTheme && newTheme !== this.personality.currentMode) {
        console.log(
          `[Personality] Syncing mode from settings: ${this.personality.currentMode} → ${newTheme}`,
        );
        this.switchMode(newTheme as PersonaMode);
      }
    });
  }
