...
export class GovernedActionRequestService {
  ...
  getRequest(requestId: string): GovernedActionRequest | undefined { return this.requests.find((item) => item.requestId === requestId); }
  markApprovedWaitingExecution(requestId: string): GovernedActionRequest | undefined { return this.update(requestId, { status: "approved_waiting_execution" }); }
  markExecutedElsewhere(requestId: string, metadata?: Record<string, unknown>): GovernedActionRequest | undefined { return this.update(requestId, { status: "executed_elsewhere", parametersPreview: metadata ? { ...this.requests.find((item) => item.requestId === requestId)?.parametersPreview, executionMetadata: metadata } : undefined }); }
  markBlocked(requestId: string, reason: string): GovernedActionRequest | undefined { return this.update(requestId, { status: "blocked", parametersPreview: { ...this.requests.find((item) => item.requestId === requestId)?.parametersPreview, blockedReason: reason } }); }
  getExecutableRequestSummary(): { total: number; approvedWaiting: number; proposed: number; approvalRequired: number } { return { total: this.requests.length, approvedWaiting: this.requests.filter((item) => item.status === "approved_waiting_execution").length, proposed: this.requests.filter((item) => item.status === "proposed").length, approvalRequired: this.requests.filter((item) => item.status === "approval_required").length }; }
