...
const POLICY_SEEDS: VisualCoreSurfacePolicy[] = [
  ...
  policy({
    ...
    riskLevel: "elevated",
    readiness: "ready_for_display_governance",
    label: "Browser (Governed)",
    summary:
      "Routes a browser surface via the governed LucaBrowser adapter/session (PR #143). No automation, no DOM read, no click/type/scroll.",
    capabilities: caps({
      displaysData: true,
      opensBrowser: true,
    ...
    notes: [
      "PR #143 — renders <LucaBrowser browserMode=\"GOVERNED\" /> via the governed adapter.",
      "Remote command BROWSER_NAVIGATE creates a governed shell session and switches to BROWSER mode.",
      "Header label reads LUCA_BROWSER_GOVERNED — governed naming.",
      "No automation, no DOM read, no click/type/scroll, no screenshot/OCR/vision.",
    ],
  }),
  policy({
    mode: "DATA",
