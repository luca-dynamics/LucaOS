...
class ModelRouterService {
  ...
  /**
   * Resolve a canonical chat route for the current settings while preserving the
   * local recommendation metadata used by task/intention UX.
   */
  public async recommendRoute(
    ...
    const route = await modelReadinessResolver.resolveRoute({
      capability: "chat",
      requestedModel:
        recommendation.status === "READY" ? recommendation.modelId : undefined,
    });

    return {
      ...recommendation,
      route,
      reason: `${recommendation.reason} Route: ${route.reason}`,
    };
  }
  ...
  private inferProfile(
    ...
    if (skillSets.includes("FINANCE") || intention.complexity >= 8)
      return "LOGIC";
    if (
      skillSets.includes("SYSTEM_ADMIN") ||
      skillSets.includes("AGENCY_EVOLUTION")
    )
      return "CODING";
