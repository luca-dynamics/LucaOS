...
try {
  ...
  console.warn("[TRADING] Luca Link Manager not available for sync");
}
...
export const tradingService = {
  ...
  async syncPositionsToMobile(exchange: string) {
    ...
    if (lucaLinkManager) {
      lucaLinkManager.syncState("portfolio_update", {
        exchange,
        positions,
        timestamp: Date.now(),
      });
    ...
    return { success: false, error: "Luca Link unavailable" };
  },
  ...
  async executeOrder(exchange: string, order: any) {
    ...
    if (!riskCheck.allowed) {
      if (lucaLinkManager) {
        lucaLinkManager.syncState("alert", {
    ...
    if (result.success || result.orderId) {
      ...
      // Sync to Mobile
      if (lucaLinkManager) {
        lucaLinkManager.syncState("trading_alert", {
          type: "execution",
          data: {
            symbol: order.symbol,
            side: order.side,
  ...
  async getLeaderboard() {
    ...
    return [
      ...
      {
        trader_id: "4",
        trader_name: "LUCA_TRADER",
        avatar: "BRAIN_CIRCUIT",
