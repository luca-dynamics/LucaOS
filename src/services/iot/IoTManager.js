...
// import hisenseProvider from './providers/HisenseProvider.js'; // REMOVED: Replaced by GenericMqttProvider
import { GenericMqttProvider } from './providers/GenericMqttProvider.js';
import { DRIVERS_DIR } from '../../../cortex/server/config/constants.js';
...
class IoTManager extends EventEmitter {
    ...
    async loadDrivers() {
        ...
        const files = fs.readdirSync(DRIVERS_DIR).filter(f => f.endsWith('.json'));
        console.log(`[IoT_MANAGER] Found ${files.length} drivers in ${DRIVERS_DIR}`);

        for (const file of files) {
            try {
                const content = fs.readFileSync(path.join(DRIVERS_DIR, file), 'utf8');
                const definition = JSON.parse(content);
                
                if (definition.connection && definition.connection.protocol.startsWith('mqtt')) {
                    const provider = new GenericMqttProvider(definition);
                    await this.registerProvider(provider);
                    console.log(`[IoT_MANAGER] Loaded Generic Driver: ${definition.name}`);
                }
            } catch (e) {
                console.error(`[IoT_MANAGER] Failed to load driver ${file}:`, e);
            }
        }
    }
    ...
    async controlDevice(deviceId, action, params = {}) {
        ...
        // [MIGRATION COMPLETE] Removed hardcoded Hisense Check.
        // The GenericMqttProvider now handles this via the loaded driver.
