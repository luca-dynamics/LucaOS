...
export const ToolSchemas = {
  ...
  batchAnalyzeAndOrganizeDirectory: z.object({
    targetPath: z
      .string()
      .describe("Absolute path to the directory containing files to organize"),
    instruction: z
      .string()
      .describe(
        "Natural language instructions on how to sort and rename the files (e.g. 'Sort by year and rename to YYYY-MM-DD_Name.pdf')",
      ),
  }),
  listFiles: z.object({
  ...
  sheets_read_range: z.object({
    spreadsheetId: z.string(),
    range: z.string(),
  }),
  sheets_write_range: z.object({
    spreadsheetId: z.string(),
    range: z.string(),
  ...
  contacts_search: z.object({
    query: z.string(),
  }),
  contacts_get: z.object({
    resourceName: z.string(),
  }),

  // --- UNIVERSAL IOT ADAPTER ---
  scanLocalDevices: z.object({
    scanType: z.enum(["FAST_DISCOVERY", "FULL_PORT_SCAN"]).optional(),
    targetSubnet: z.string().optional(),
  }),
  researchDeviceProtocol: z.object({
    brand: z.string(),
  ...
  generateDeviceDriver: z.object({
    deviceName: z.string(),
    protocol: z.enum(["HTTP", "TCP", "UDP", "MQTT", "BLE"]),
    definition: z.string().describe("Stringified JSON driver definition"),
  }),
  executeDeviceCommand: z.object({
    target: z.string(),
    command: z.string(),
