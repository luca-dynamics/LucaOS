...
class LucaLinkServerManager extends EventEmitter {
    ...
    async delegateTool(deviceId, toolName, args) {
        return new Promise((resolve, reject) => {
            ...
            console.log(`[LUCA_LINK] Delegating tool '${toolName}' to device ${deviceId} (CMD: ${commandId})`);
    ...
    resolveCommand(commandId, result) {
        if (this.pendingCommands.has(commandId)) {
            ...
            this.pendingCommands.delete(commandId);
            resolve(result);
            console.log(`[LUCA_LINK] Command ${commandId} resolved successfully.`);
        } else {
            console.warn(`[LUCA_LINK] Received response for unknown command: ${commandId}`);
        }
    }
    ...
    rejectCommand(commandId, error) {
        if (this.pendingCommands.has(commandId)) {
            ...
            console.warn(`[LUCA_LINK] Command ${commandId} rejected: ${error}`);
        }
    }
    ...
    registerDevice(socket, metadata) {
        ...
        console.log(`[LUCA_LINK_SERVER] Registered device: ${deviceId}`);
    ...
    unregisterDevice(deviceId) {
        if (this.devices.has(deviceId)) {
            ...
            console.log(`[LUCA_LINK_SERVER] Unregistered device: ${deviceId}`);
...
export const lucaLinkManager = new LucaLinkServerManager();
