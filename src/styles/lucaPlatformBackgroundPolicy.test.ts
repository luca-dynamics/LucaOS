...
describe("lucaPlatformBackgroundPolicy", () => {
  it("resolves mobile web viewports to the stable mobile system background policy", () => {
    ...
    expect(policy.supportsTrueOsTransparency).toBe(false);
  });
  ...
  it("does not claim true OS transparency for desktop web", () => {
    const policy = resolveLucaPlatformBackgroundPolicy({
      isMobileViewport: false,
      isNativeMobile: false,
      isDesktopNative: false,
  ...
  it("resolves Electron/native desktop to the native desktop material-ready policy", () => {
    const policy = resolveLucaPlatformBackgroundPolicy({
      isMobileViewport: false,
      isNativeMobile: false,
    ...
    expect(policy.supportsTrueOsTransparency).toBe(true);
  });
