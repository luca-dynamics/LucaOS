...
export const SettingsLucaLinkRuntimeAuthority: React.FC<{ accentColor: string }> = ({ accentColor }) => (
  ...
  >
    ...
    <div className="mt-3 rounded-lg border p-3" style={{ borderColor: settingsSurfaceTokens.borderSubtle }}>
      <p className="text-xs font-semibold">Runtime flags</p>
      <div className="mt-2 grid grid-cols-2 gap-1 text-xs md:grid-cols-4">
        {runtimeFlags.map(([label, enabled]) => <p key={label}>{label}: <strong>{String(enabled)}</strong></p>)}
      </div>
    </div>
