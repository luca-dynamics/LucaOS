...
const InstagramManager: React.FC<Props> = ({ onClose, theme }) => {
  ...
  const checkStatus = async () => {
    try {
      const res = await fetch(apiUrl("/api/instagram/status"));
  ...
  const handleLogout = async () => {
    if (confirm("Disconnect Instagram session?")) {
      setLoading(true);
      try {
        await fetch(apiUrl("/api/instagram/logout"), { method: "POST" });
        setStatus((prev) => ({
          ...prev,
          status: "OFFLINE",
          hasSession: false,
          ...
          >
            {status.hasChromeProfile ? (
              <>
                ...
                Your imported Chrome data includes Instagram sessions.
              </>
            ) : (
              <>
