...
const LinkedInManager: React.FC<Props> = ({ onClose, theme }) => {
  const isLight = theme?.isLight || theme?.themeName?.toLowerCase() === "lucagent";
  const themeHex = theme?.hex || "#0A66C2";
  ...
  const checkStatus = async () => {
    try {
      const res = await fetch(apiUrl("/api/linkedin/status"));
  ...
  const handleLogout = async () => {
    if (confirm("Disconnect LinkedIn session?")) {
      setLoading(true);
      try {
        await fetch(apiUrl("/api/linkedin/logout"), { method: "POST" });
        setStatus((prev) => ({
          ...prev,
          status: "OFFLINE",
          hasSession: false,
          ...
          >
            {status.hasChromeProfile ? (
              <>
                ...
                Your imported Chrome data includes LinkedIn sessions.
              </>
            ) : (
              <>
