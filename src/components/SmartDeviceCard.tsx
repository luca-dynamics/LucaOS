...
const SmartDeviceCard: React.FC<Props> = ({
  device,
  onControlClick,
  themeColor = "text-rq-blue",
  themeBorder = "border-rq-blue",
  themeBg = "bg-rq-blue-dim",
  themeName = "default",
}) => {
  const getIcon = () => {
    switch (device.type) {
  ...
  const activeColor =
    device.status === "error"
      ? "text-rq-red border-rq-red bg-rq-red-dim"
      : device.isOn
        ? `${themeColor} ${themeBorder} ${themeBg}`
        : isLight
          ? "text-gray-500 border-gray-200 bg-white/30"
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center gap-1">
          <div
            className={`w-2 h-2 rounded-full ${device.status === "online" ? (isLight ? "bg-cyan-500" : "bg-green-400") : "bg-red-500"} animate-pulse`}
          />
      ...
      <div>
        <h3
          className={`font-display font-bold text-lg tracking-wide truncate ${isLight ? "text-gray-900" : ""}`}
        >
          {device.name}
        </h3>
        <p
          className={`font-mono text-xs opacity-60 ${isLight ? "text-gray-600" : ""}`}
        >
          {device.location}
        </p>
      </div>
      ...
      <div
        className={`mt-2 flex items-center justify-between font-mono text-xs ${isLight ? "text-gray-700" : ""}`}
      >
        <span>STATUS:</span>
        <span
          className={`font-bold ${device.isOn ? (isLight ? "text-cyan-700" : "text-white") : "opacity-50"}`}
        >
      ...
      {device.type === DeviceType.SMART_TV && onControlClick && (
        <button
          ...
          className={`mt-2 w-full py-1 text-[10px] font-bold tracking-widest transition-colors group/btn ${isLight ? "text-gray-800" : ""}`}
          style={{
            background: isLight
          ...
          onMouseEnter={(e) => {
            ...
            let hoverBg = isLight
              ...
              : "rgba(30, 58, 138, 0.4)"; // default blue
            if (!isLight) {
              if (themeBg.includes("blue-950"))
                hoverBg = "rgba(30, 58, 138, 0.4)";
              else if (themeBg.includes("gray-950"))
                hoverBg = "rgba(3, 7, 18, 0.4)";
              else if (themeBg.includes("green-950"))
                hoverBg = "rgba(5, 46, 22, 0.4)";
              else if (themeBg.includes("rgba(201,118,61"))
                hoverBg = "rgba(201, 118, 61, 0.4)";
            }
            e.currentTarget.style.backgroundColor = hoverBg;
            if (isLight) e.currentTarget.style.color = "#0891b2";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.backgroundColor = "";
            if (isLight) e.currentTarget.style.color = "";
          }}
        >
          LAUNCH REMOTE
        </button>
