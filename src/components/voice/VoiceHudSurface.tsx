...
export interface VoiceHudSurfaceProps {
  ...
  enableVoiceTelemetryDebug?: boolean;
  isTacticalTelemetryMode?: boolean;
...
export function VoiceHudSurface({
  ...
  hideControls = false,
  enableVoiceTelemetryDebug = import.meta.env.VITE_LUCA_SHOW_VOICE_TELEMETRY ===
    "true",
  ...
  const showTechnicalPanels =
    ...
    (isTacticalTelemetryMode ?? isTactical);
  const palette =
  ...
  const missionColor = hasMission
    ? MISSION_COLORS[colorKey as keyof typeof MISSION_COLORS]
    : "rgb(239, 68, 68)";
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 left-0 w-full h-full pointer-events-none transition-opacity duration-500 ${isVideoActive ? "opacity-40" : "opacity-0"}`}
      >
        <video
          ref={videoRef}
          autoPlay
        ...
        <div className="absolute top-[15%] w-full flex justify-center z-40 animate-in fade-in zoom-in-95 duration-500 pointer-events-none">
          ...
          >
            <TacticalStream
              ...
              title={visualData.title || "Voice context"}
        ...
        <div className="text-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 min-h-[60px] sm:min-h-[80px] w-full max-w-[90vw] sm:max-w-full">
          <div className="mb-2 sm:mb-3 flex items-center justify-center gap-2">
            ...
            <span className="text-[10px] tracking-[0.4em] font-bold uppercase text-[var(--app-text-main)] opacity-80">
              {transcriptSource === "model" ? "Luca" : "Listening"}
            </span>
          </div>
          {transcript ? (
            ...
            <div className="text-[var(--app-text-muted)] font-mono text-[10px] sm:text-xs animate-pulse">
              {micAvailable
                ? "Waiting for your voice…"
                : realtimeLastError || "Voice access is not available yet."}
            </div>
          )}
          {statusMessage && (
            <div
          ...
          {showTypedFallback && (
            <div className="pointer-events-auto mx-auto mt-5 w-full max-w-2xl text-left">
              <textarea
                id="voice-hud-typed-fallback"
                value={typedFallbackValue}
                onChange={(event) =>
                  onTypedFallbackChange?.(event.target.value)
                }
                rows={3}
      ...
      {showTechnicalPanels && (
        <div className="absolute left-16 bottom-[15%] hidden md:flex flex-col gap-4 w-64 font-mono text-[10px] z-10 pointer-events-none">
          ...
          >
            <Icon name="Programming" size={14} variant="Linear" /> ACTIVE
            PROTOCOLS
          </div>
          <div className="space-y-3 text-[var(--app-text-muted)]">
            ...
            <div className="text-[8px] opacity-30 pt-2">
              ...AND {Math.max(0, totalToolCount - dynamicProtocols.length)}{" "}
              MORE MODULES
            </div>
          </div>
        </div>
      ...
      {showTechnicalPanels && (
        <div className="absolute right-16 bottom-[15%] hidden md:flex flex-col gap-2 w-80 font-mono text-[10px] text-right z-30 pointer-events-auto">
          <div className="font-bold mb-2" style={{ color: themeColor }}>
            TELEMETRY STREAM
          </div>
          ...
          <div
            ...
            style={{
              borderColor: hasMission
                ? `${missionColor}88`
                : "rgba(239, 68, 68, 0.3)",
              backgroundColor: hasMission
                ? `${missionColor}22`
                : "rgba(127, 29, 29, 0.1)",
              color: missionColor,
      ...
      <div className="absolute bottom-4 md:bottom-8 flex flex-wrap items-center justify-center gap-3 md:gap-12 text-[8px] md:text-[10px] font-mono text-[var(--app-text-muted)] uppercase tracking-widest z-[60] pointer-events-none px-4 w-full text-center">
        <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap">
          <Icon
            name="VolumeUp"
            size={10}
            className={`md:w-3 md:h-3 ${amplitude > 0.5 ? "text-[var(--app-text-main)]" : ""}`}
            variant="Linear"
          />
          VOL:{" "}
        ...
        <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap">
          <Icon
            ...
            variant="Linear"
          />
          Voice {(realtimeStatus || "idle").toLowerCase()}
        </div>
        <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap">
          <Icon
            name="ShieldTick"
            ...
            variant="Linear"
          />
          Route {(runtimeRouteHealth || "stable").toLowerCase()}
          {runtimeFallbackActive ? " · typing available" : ""}
        </div>
        <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap">
          <Icon
            name="Timer1"
            ...
            variant="Linear"
          />
          RTT: {runtimeLatency ?? 0}ms
        </div>
        {realtimeLastError ? (
          <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap text-[var(--luca-danger,#f87171)]">
            {realtimeLastError}
          </div>
        ) : null}
        {!realtimeCanInterrupt ? (
          <div className="flex items-center gap-1 md:gap-2 whitespace-nowrap text-[var(--luca-warning,#f2b23e)]">
            Finishing response
          </div>
        ) : null}
      </div>
