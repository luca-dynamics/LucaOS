...
const FaceScan: React.FC<FaceScanProps> = ({
  ...
  const borderCard = isLightTheme ? "border-slate-300" : "border-white/10";
  const bgIcon = isLightTheme ? "bg-slate-200" : "bg-white/10";
  const borderIcon = isLightTheme ? "border-slate-400" : "border-white/20";
  ...
  if (!cameraAvailable) {
    return (
      <div className="space-y-6 animate-fade-in-up w-full max-w-2xl text-center">
        <div
          className={`w-20 h-20 mx-auto ${bgIcon} border ${borderIcon} rounded-full flex items-center justify-center backdrop-blur-xl`}
        >
          <Camera className={`w-10 h-10 ${textMuted}`} />
        </div>
      ...
      <div className="space-y-6 animate-fade-in-up w-full max-w-2xl">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            ...
            <div
              className={`${isLightTheme ? "bg-white/80" : "bg-black/60"} backdrop-blur-sm px-6 py-3 rounded-lg border`}
              style={{
                borderColor: theme ? setHexAlpha(theme.hex, 0.5) : undefined,
              }}
            >
  ...
  return (
    ...
    >
      ...
      {!hideHeader && (
        <div className={`text-center ${compact ? "space-y-0.5" : "space-y-4"}`}>
          <div
            ...
            } mx-auto ${bgIcon} border rounded-full flex items-center justify-center backdrop-blur-xl transition-all`}
            style={{
              borderColor: theme
          ...
          <h2
            ...
            } font-bold uppercase tracking-widest ${textHeading} transition-all`}
          >
            {title}
          </h2>
      ...
      <div
        className={`${bgCard} border ${borderCard} rounded-xl ${
          compact
            ? isMobile
              ? "p-1.5 space-y-0.5"
              : "p-3 space-y-1.5"
