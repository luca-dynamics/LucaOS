...
export function fromWidgetUpdatePayload(payload: LegacyPresencePayload): PresenceSnapshot {
  ...
  const state: PresenceRuntimeState = {
    ...
    voice: {
      ...
      isVadActive: voice.isVadActive ?? voice.isListening ?? false,
      isSpeaking: voice.isSpeaking ?? false,
      amplitude: voice.amplitude ?? 0,
      ...(voice.source !== undefined ? { source: voice.source } : {}),
      ...(voice.provider !== undefined ? { provider: voice.provider } : {}),
      ...(voice.model !== undefined ? { model: voice.model } : {}),
      ...(voice.persona !== undefined ? { persona: voice.persona } : {}),
      ...(voice.language !== undefined ? { language: voice.language } : {}),
      ...(voice.fallbackReason !== undefined ? { fallbackReason: voice.fallbackReason } : {}),
      ...(voice.error !== undefined ? { error: voice.error } : {}),
      ...(voice.timestamp !== undefined ? { timestamp: voice.timestamp } : {}),
      ...(voice.requestId !== undefined ? { requestId: voice.requestId } : {}),
      ...(voice.sessionId !== undefined ? { sessionId: voice.sessionId } : {}),
    },
...
export function toWidgetUpdatePayload(snapshot: PresenceSnapshot): LegacyPresencePayload {
  return {
    ...toLegacyVoiceUpdatePayload({
      ...
      isVadActive: snapshot.voice.isVadActive ?? snapshot.voice.isListening,
