...
import type { CanonicalProviderId } from "../shared/llm/providerIds.js";
...
export interface LucaProviderHubEntry {
  ...
  /**
   * The provider id the core's gateway routes by, or `null` when the core cannot
   * reach this provider at all.
   *
   * These display ids are a vendor-family vocabulary for the UI; the core speaks
   * a shorter one (`src/shared/llm/providerIds.js`). The mapping used to be
   * implicit, which meant a reader had no way to tell that picking "Together AI"
   * here cannot produce a call — five entries are `null`, and being able to see
   * that is the point of the field.
   *
   * Not injective: `luca_prime` and `google_gemini` both map to `gemini`, because
   * Luca Prime is Gemini-backed today.
   */
  readonly canonicalProviderId: CanonicalProviderId | null;
