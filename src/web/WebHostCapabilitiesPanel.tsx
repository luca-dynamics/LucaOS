...
export function WebHostCapabilitiesPanel({ onOpenLucaLink }: { onOpenLucaLink: () => void }) {
  ...
  return (
    <div data-settings-surface="host-capabilities" className="grid gap-4">
      <ExtractedSettingsSection eyebrow="System Status" title="Host & Capabilities" description={`Current host: ${runtime.hostClass.replace("-", " ")}. Review browser capabilities and governed routes only when needed.`}>
        <ExtractedAction onClick={onOpenLucaLink}>Open LucaLink</ExtractedAction>
      </ExtractedSettingsSection>
      ...
      <ExtractedSettingsSection title="Route Unlock" description="Browser permissions, installed hosts, paired hosts, approved connectors, governed routes, and API configuration remain explicit and contextual." />
    </div>
  );
}
