import type {
  PersonalIntelligenceRuntimeAuthorityReadiness,
  PersonalIntelligenceRuntimeAuthorityRecord,
} from "./runtimeAuthorityTypes";

export function summarizePersonalIntelligenceRuntimeAuthority(
  records: readonly PersonalIntelligenceRuntimeAuthorityRecord[],
): PersonalIntelligenceRuntimeAuthorityReadiness {
  const count = (value: PersonalIntelligenceRuntimeAuthorityRecord["authorityClass"]) =>
    records.filter((record) => record.authorityClass === value).length;
  return {
    totalRecords: records.length,
    permanentlyBlocked: count("permanently_blocked"),
    reviewOnly: count("review_only"),
    dryRunOnly: count("dry_run_only"),
    futurePilotCandidates: count("future_pilot_candidate"),
