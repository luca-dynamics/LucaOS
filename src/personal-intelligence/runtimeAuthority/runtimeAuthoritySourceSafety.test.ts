...
const forbidden = [
  /memoryService/, /saveMemory\s*\(/, /governedMemoryAdapter/, /live.?write helper/i,
  /services\/lucaLink\//i, /lucaLinkService\.send/, /modelRouter|providerRuntime/i,
  /mcp.*execute|tool.*execute|workflow.*execute/i, /from\s+["'](?:node:)?fs(?:\/promises)?["']/,
  /child_process/, /\bfetch\s*\(/, /WebSocket|socket\.io-client/, /localStorage|sessionStorage|indexedDB/,
  /ipcRenderer|ipcMain/, /playwright|puppeteer/, /import\s*\(.*entrypointRef/i, /\beval\s*\(/,
  /new\s+Function\s*\(/, /new\s+Worker\s*\(/, /from\s+["'](?:node:)?vm["']/,
  /VisualCore.*(?:set|mutate|update)|LucaBrowser.*(?:set|mutate|update)/i,
];

describe("runtime authority source safety", () => {
  it.each(forbidden)("does not use forbidden runtime API %s", (pattern: RegExp) => {
