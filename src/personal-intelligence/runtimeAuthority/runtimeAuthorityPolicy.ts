...
const CAPABILITIES = new Set<PersonalIntelligenceRuntimeCapabilityKind>([
  "skill_execution", "tool_invocation", "mcp_invocation", "workflow_execution", "model_call", "memory_proposal", "memory_write",
  "browser_action", "network_access", "file_read", "file_write", "connector_access", "lucalink_handoff", "shell_command",
  "install_package", "credential_access", "payment_or_trading", "device_control", "generated_code_execution", "private_reasoning_access", "unknown",
]);
const SOURCES = new Set<PersonalIntelligenceRuntimeAuthoritySource>([
  "skill_registry", "sandbox_plan", "permission_gates", "dry_run", "memory_proposal", "mission_profile", "runtime_trace", "fixture",
]);
const PERMANENT = new Set<PersonalIntelligenceRuntimeCapabilityKind>([
  "shell_command", "install_package", "credential_access", "private_reasoning_access", "generated_code_execution", "device_control", "payment_or_trading",
]);
const REVIEW = new Set<PersonalIntelligenceRuntimeCapabilityKind>(["memory_proposal"]);
const DRY_RUN = new Set<PersonalIntelligenceRuntimeCapabilityKind>([
  "skill_execution", "tool_invocation", "mcp_invocation", "workflow_execution", "model_call", "memory_write", "browser_action",
  "network_access", "file_read", "file_write", "connector_access", "lucalink_handoff",
]);
const PILOT_EVIDENCE = [
  "successful dry-run evidence", "all required gates granted for review", "aligned or reviewed mission evaluation",
  "rollback expectation", "runtime trace preview", "absence of permanently blocked capabilities",
];
const RUNTIME_BOUNDARY = ["isolated runtime", "rollback enforcement", "durable audit", "explicit human review", "separate pilot implementation"];
const BLOCKED_ACTIONS = ["skill execution", "tool invocation", "model call", "memory write", "LucaLink handoff", "runtime authority grant"];
