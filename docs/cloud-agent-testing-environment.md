...
## Recommended cloud-agent validation order
1. Run `npm ci`.
...
When install fails in cloud/ephemeral agent environments, report clearly:
...
Example wording:
- "`npm ci` failed in a cloud/ephemeral coding agent environment during native dependency build (`robotjs` via `node-gyp`) due to missing Python `distutils`; as a result `vitest` was unavailable and scoped `src/services/computerUse` tests could not run in this environment."
