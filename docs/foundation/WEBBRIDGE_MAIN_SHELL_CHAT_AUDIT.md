...
The central surface is `src/web/chat/WebRealChatPanel.tsx`. It mounts the
shared desktop `ChatPanel` and owns only browser-safe conversation state. All
model execution is routed through `webAppRuntime.chat`, which delegates to the
narrow adapter in `src/web/chat/webChatRuntime.ts`. The current adapter imports
no provider SDK, service, secret, or native runtime.
