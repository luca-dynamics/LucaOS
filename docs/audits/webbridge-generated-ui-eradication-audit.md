...
| `src/web/WebLucaShell.tsx`                             | thin dashboard adapter          | Previously risked owning dashboard shell slots.             | Kept routed through `LucaDashboardSurface` with adapter-provided slots and `WebRealChatPanel`.                                                                 | Replace temporary slot copy with richer shared dashboard props in later parity work. |
...
| `src/web/chat/WebRealChatPanel.tsx`                    | thin chat runtime adapter       | Must not own chat presentation.                             | Confirmed it renders `ChatPanel` and only maps runtime state/props.                                                                                            | None.                                                                                |
...
- `src/web/chat/WebRealChatPanel.tsx` renders `ChatPanel`.
...
`ChatPanel` now composes the original MiniChat child components: `ChatWidgetHistory`, `ChatWidgetInput`, and `SuggestionChips`. The WebBridge adapter does not render chat bubbles, textareas, or primary chat chrome directly.
