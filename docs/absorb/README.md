...
| --- | --- | --- |
| **1 Stability + execution** | **Partial → real wire + UI** | Gates + tape; product complete API; workforce **starts** MissionControl + maps goals + **live IN_PROGRESS/complete sync** + gated complete; **CU defaults mission tape on**, links MissionControl, **maps step goals** (IN_PROGRESS → done); **Mission Center** live poll, readiness, blocked-gate detail, **tape step timeline (replay pilot)**; PI Mission Profile stays **read-only advisory** |
