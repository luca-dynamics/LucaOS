...
| **2 Memory + context** | **Partial → vault pilot** | PI memory / approval pilots; **MemoryVaultService** list/edit/export/import; **ingest** (device/app/chat/lucalink); **compress** (dedupe/truncate); **multi-format import** (ChatGPT/Claude/array); Memory Vault UI; deeper auto-scrape + model summarization open |
