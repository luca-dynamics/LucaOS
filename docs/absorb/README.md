...
| --- | --- | --- |
| **1 Stability + execution** | **Partial → real wire + UI** | Gates + tape; product complete API; workforce **starts** MissionControl + maps goals + gated complete; **CU defaults mission tape on**, links MissionControl, **maps step goals** (IN_PROGRESS → done); **Mission Center** live poll + source label; PI Mission Profile stays **read-only advisory** |
