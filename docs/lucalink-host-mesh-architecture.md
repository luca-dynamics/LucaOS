...
1. **`lucaLinkService.ts` → `lucaLink` singleton (the active runtime).** A
   Socket.IO client that connects a Desktop "room" to Mobile/Guest clients via
...
display hosts. Instead of behaving like separate desktop/mobile apps, LucaOS
should behave like *one Luca* that can sense, think, and act through whichever
...
| `guest-connected` | relay→desktop | Guest attached. |
| `guest-message` | relay→desktop | Guest chat/auth message. |
| `desktop-to-guest` | desktop→relay | Desktop response/audio to guest. |
| `desktop-message` | relay→guest | Delivered desktop output. |
| `guest-disconnected` | relay→desktop | Guest left. |
...
interface LucaLinkDevice { deviceId: string; type: string; name: string; lastSeen: number; }

interface LucaLinkMessage { // service variant
  id: string; type: string; source: string; target: string;
  timestamp: number; payload?: unknown; secure?: boolean;
...
interface Device {
  ...
  platform: "ios" | "android" | "web" | "macos" | "windows" | "linux" | "tizen" | "webos" | "wearos";
  capabilities: string[]; status: "online" | "offline" | "away";
  lastSeen: Date; trustLevel: number; // 0-100
  metadata: DeviceMetadata; publicKey?: string; identityPublicKey?: string;
}

interface Session {
  sessionId: string; deviceId: string; sharedSecret: string; // encrypted
  publicKey: string; lastSeen: number; capabilities: string[];
  preferences: Record<string, any>;
}

interface EncryptedMessage { iv; encrypted; signature; timestamp; nonce; }
interface KeyPair { publicKey; secretKey; } // Uint8Array
interface SharedSecret { key: Uint8Array; createdAt; expiresAt; }
```
...
interface LucaHostManifest {
  ...
  sensors: Array<
    "camera" | "microphone" | "location" | "accelerometer"
    | "lidar" | "temperature" | "presence"
  >;
...
The Host Router answers question #5: *should Luca route this task through this
host?* This is a **design-only** scoring model (not implemented in this PR).
...
The current tab already has *Linked Devices*, *Pair New Device*, *Sync
Behavior*, *Access Control*, and *Advanced Details* sections — these map forward
to:
