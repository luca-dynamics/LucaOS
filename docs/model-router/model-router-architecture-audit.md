...
This PR adds a static audit inventory and a lightweight future contract in `src/model-router/` without changing runtime provider selection.
...
3. **Add provider capability registry** — include provider aliases, model capabilities, key requirements, cost tier, latency posture, and supported task types.
