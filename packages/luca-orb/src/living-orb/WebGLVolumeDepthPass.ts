...
export interface CanonicalVolumeMesh {
...
const DEFAULT_RADIAL_RINGS = 48;
const DEFAULT_ANGLE_SEGMENTS = 128;
const TAU = Math.PI * 2;

/**
 * Tessellate the authored identity volume into independent front and rear
 * surfaces. The rim is shared spatially, while the unequal Z profiles keep
 * the result from reading as a mirrored procedural lens.
 */
export function buildCanonicalVolumeMesh(
  volume: CanonicalOrbVolume = CANONICAL_LUCA_VOLUME_V2,
  radialRings = DEFAULT_RADIAL_RINGS,
  angleSegments = DEFAULT_ANGLE_SEGMENTS,
): CanonicalVolumeMesh {
  if (radialRings < 2) throw new Error('Canonical volume mesh requires at least two radial rings.');
  if (angleSegments < 12) throw new Error('Canonical volume mesh requires at least twelve angle segments.');
  ...
  for (const surface of [1, -1] as const) {
    ...
    const center = volume.outer.center;
    const centerHalfDepth = sampleOrbHalfDepth(volume.depth, 0, center);
    const centerZ = centerHalfDepth * (surface > 0 ? volume.depth.frontScale : -volume.depth.rearScale);
    ...
    for (let ring = 1; ring <= radialRings; ring += 1) {
      ...
      for (let segment = 0; segment < angleSegments; segment += 1) {
        ...
        const contourRadius = sampleOrbContour(volume.outer, angle);
        const x = center[0] + Math.cos(angle) * contourRadius * radial;
        const y = center[1] + Math.sin(angle) * contourRadius * radial;
        const halfDepth = sampleOrbHalfDepth(volume.depth, radial, [x, y]);
        const z = halfDepth * (surface > 0 ? volume.depth.frontScale : -volume.depth.rearScale);
    ...
    if (vertices.length / 4 !== base + verticesPerSurface) {
      throw new Error('Canonical volume mesh vertex layout is inconsistent.');
    }
  }

  if (vertices.length / 4 > 0xffff) {
    throw new Error('Canonical volume mesh exceeds the 16-bit index envelope.');
  }
...
/** Internal render pass; callers only provide the existing common uniforms. */
export class WebGLVolumeDepthPass {
  ...
  constructor(private readonly gl: WebGL2RenderingContext) {
    const mesh = buildCanonicalVolumeMesh();
    this.indexCountPerSurface = mesh.indices.length / 2;

    const program = this.createProgram(DEPTH_VERT, DEPTH_FRAG);
    const vao = gl.createVertexArray();
    const vertexBuffer = gl.createBuffer();
    const indexBuffer = gl.createBuffer();
    if (!vao || !vertexBuffer || !indexBuffer) throw new Error('Failed to allocate canonical volume mesh.');
    ...
    const surfaceLocation = gl.getAttribLocation(program, 'a_surface');
  ...
  private createProgram(vertexSource: string, fragmentSource: string): WebGLProgram {
    ...
    const compile = (source: string, type: number): WebGLShader => {
      ...
      if (!shader) throw new Error('Failed to create canonical volume shader.');
      gl.shaderSource(shader, source);
      gl.compileShader(shader);
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        const info = gl.getShaderInfoLog(shader);
        gl.deleteShader(shader);
        throw new Error(`Canonical volume shader compile error:\n${info}`);
      }
      return shader;
    };
    ...
    if (!program) throw new Error('Failed to create canonical volume program.');
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      ...
      throw new Error(`Canonical volume program link error:\n${info}`);
    }
    return program;
  }
}
