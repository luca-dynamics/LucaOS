...
class MacOSAdapter(PlatformAdapter):
    ...
    def check_permissions(self) -> Dict[str, Any]:
        ...
        # If we successfully verified permissions, don't check via screencapture again for 30 mins.
        # If we failed, retry every 30 seconds to detect when user grants access.
        is_success = self._permission_cache and self._permission_cache.get("success")
        ttl = 1800 if is_success else self.CACHE_TTL

        if self._permission_cache and (current_time - self._last_check_time < ttl):
