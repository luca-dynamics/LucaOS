...
import { CHROME_PROFILES_DIR as LUCA_BROWSER_PROFILE_DIR } from '../config/constants.js';
...
export async function importChromeProfile(profileName = 'Default') {
    ...
    // Create Luca browser profile directory and Partition profile folder
    const destProfileRoot = LUCA_BROWSER_PROFILE_DIR;
    const destProfilePartition = path.join(destProfileRoot, 'Partitions', 'luca');
    
    if (!fs.existsSync(destProfilePartition)) {
        fs.mkdirSync(destProfilePartition, { recursive: true });
    }

    // Key files/folders to copy (not everything - just what's needed for sessions)
    // IMPORTANT: Exclude 'Preferences' and 'Secure Preferences' to avoid clobbering Electron's own app state.
    const itemsToCopy = [
        'Cookies',
        'Login Data',
        'Web Data',
    ...
    for (const item of itemsToCopy) {
        ...
        const dest = path.join(destProfilePartition, item);
