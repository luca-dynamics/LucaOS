...
import chromeProfileService from '../../services/chromeProfileService.js';
...
router.post('/execute', (req, res) => {
    ...
    if (securityManager.isGodMode() && !isBypassed) {
        if (!isCommandSafe(command)) {
            ...
            return res.status(403).json({ 
                ...
                metadata: { command },
                message: 'Autonomous command execution is restricted. Manual safety bypass required.' 
            });
        }
    }
    ...
    // Basic filter for truly dangerous strings even in God Mode (Defense in Depth)
    const blocked = ['rm -rf /', ':(){ :|:& };:'];
    if (blocked.some(b => command.includes(b))) {
        return res.status(403).json({ error: 'Command blocked by safety filter.' });
    }

    safeExec(command, (err, stdout, stderr) => {
         if (err) return res.json({ error: err.message, stderr });
         res.json({ result: stdout.trim(), stderr });
    });
});
...
router.post('/script', (req, res) => {
    ...
    if (securityManager.isGodMode() && !isBypassed) {
        ...
        return res.status(403).json({ 
            ...
            metadata: { language, scriptLength: script.length },
            message: 'Autonomous script execution is restricted. Manual safety bypass required.' 
        });
    }
...
// --- SCREEN CAPTURE (FIX 404) ---
router.get('/screenshot', async (req, res) => {
    try {
        console.log('[SYSTEM] Capture request received');
        ...
        if (capture.success && capture.imageBuffer) {
            ...
            console.error('[SYSTEM] Capture failed:', capture.error);
            res.status(500).json({
                success: false,
                error: capture.error || 'Capture failed without error message'
            });
        }
    } catch (error) {
        console.error('[SYSTEM] Screenshot route error:', error);
...
// --- SOCIAL SETTINGS ---
const SOCIAL_SETTINGS_FILE = path.join(DATA_DIR, 'social-settings.json');

router.get('/social-settings', (req, res) => {
    try {
        if (fs.existsSync(SOCIAL_SETTINGS_FILE)) {
            const settings = JSON.parse(fs.readFileSync(SOCIAL_SETTINGS_FILE, 'utf8'));
...
router.post('/social-settings', (req, res) => {
    try {
        ...
        const dir = path.dirname(SOCIAL_SETTINGS_FILE);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        
        fs.writeFileSync(SOCIAL_SETTINGS_FILE, JSON.stringify(settings, null, 2));
        res.json({ success: true });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.get('/social/status', async (req, res) => {
    try {
        const status = {
            ...
            wechat: wechatService.getStatus(),
            google: await googleService.getStatus(),
            linkedin: linkedinService.getStatus(),
            discord: discordService.getStatus(),
            youtube: youtubeService.getStatus(),
            chrome: chromeProfileService.getImportStatus()
        };
        res.json(status);
    } catch (e) {
        res.status(500).json({ error: e.message });
