...
// OSINT Identity Search (Proxy to Python Cortex)
router.post('/identity', async (req, res) => {
    const { username, email, method } = req.body;
    console.log(`[OSINT] Proxying Identity Search: ${username || email}`);

    try {
        const { CORTEX_PORT } = await import('../../config/constants.js');
        const response = await fetch(`http://localhost:${CORTEX_PORT}/api/osint/identity`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username, email, method: method || 'quick' })
        });
