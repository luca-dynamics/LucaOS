...
/**
 * CAMERA SENTRY ANALYSIS ENDPOINT
 * Analyzes webcam frames for security events
 */
router.post('/sentry-analyze', async (req, res) => {
    const { image, mode, customInstruction, model } = req.body; // Expecting base64 image

    if (!image) {
        return res.status(400).json({ status: 'error', message: 'Missing image data' });
    }

    try {
        // Log sparingly to avoid spam
        // console.log(`[SENTRY] Analyzing frame... Mode: ${mode || 'DEFAULT'}`);

        const events = await visionAnalyzerService.analyzeScreen(image, { 
            mode: mode || 'ROOM_GUARD', 
            customInstruction,
            model: model // Pass configured model
        });
        ...
        console.error('[SENTRY] Analysis error:', error);
            ...
            try {
                ...
                const response = await fetch(`${UITARS_URL}/analyze`, {
                    ...
                    body: JSON.stringify({ 
                        screenshot, 
                        instruction,
                        model: req.body.model // Forward model selection
                    }),
...
// ===== STATE MANAGEMENT =====
// Track if the vision service is actively running
let visionServiceRunning = false;

/**
 * Start vision service (manual control)
 */
router.post('/start', (req, res) => {
    visionServiceRunning = true;
    console.log('[VISION] Service started');
    res.json({ success: true, running: true });
});

/**
 * Stop vision service (manual control)
 */
router.post('/stop', (req, res) => {
    visionServiceRunning = false;
