...
import { pentestSessionStore } from '../../services/pentestSessionStore.js';
...
// --- PENTEST SESSION API (Phase 3) ---

router.get('/sessions', (req, res) => {
    try {
        const sessions = pentestSessionStore.getActiveSessions();
        res.json(sessions);
    } catch (e) {
...
router.get('/sessions/:id', (req, res) => {
    try {
        const session = pentestSessionStore.getSession(req.params.id);
...
router.post('/sessions/:id/update', (req, res) => {
    try {
        pentestSessionStore.updateSession(req.params.id, req.body);
...
router.get('/sessions/:id/findings', (req, res) => {
    try {
        const findings = pentestSessionStore.getFindings(req.params.id);
        res.json(findings);
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post('/sessions/:id/findings', (req, res) => {
    try {
        const finding = { ...req.body, sessionId: req.params.id };
        pentestSessionStore.saveFinding(finding);
        res.json({ success: true });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
