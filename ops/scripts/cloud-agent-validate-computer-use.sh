...
if ! npm ci; then
  ...
  echo "[cloud-agent-validate] In cloud/ephemeral coding agent environments this may be caused by missing Python distutils, which prevents Vitest/TS tooling installation."
  exit 1
fi
