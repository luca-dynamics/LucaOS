function registerSandboxIpc({ ipcMain, broker }) {
    ...
    ipcMain.handle('sandbox:listSnapshots', (_event, sessionId) => broker.listSnapshots(sessionId));
    ipcMain.handle('sandbox:snapshot', (_event, sessionId) => broker.snapshot(sessionId));
    ipcMain.handle('sandbox:cleanupExpired', () => broker.cleanupExpired());
