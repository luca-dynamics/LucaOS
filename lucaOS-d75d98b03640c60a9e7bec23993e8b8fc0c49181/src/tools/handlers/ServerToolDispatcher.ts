...
const SERVER_TOOLS = [
  ...
  "likeYouTubeVideo",
  "commentYouTubeVideo",
  "startSocialSkillRecording",
  "stopSocialSkillRecording",
  "listSocialSkills",
  "deleteSocialSkill",
  "osintGoogleDork",
  "readClipboard",
...
export class ServerToolDispatcher {
  ...
  static async execute(
    ...
    // --- SPECIAL LOGIC: GMAIL ATTACHMENTS ---
    if (name === "gmail_send_message") {
      let attachmentToSend = attachedImage;
      if (!attachmentToSend) {
        const reversedMsgs = [...messages].reverse();
        const lastMediaMsg = reversedMsgs.find(
          (m: any) => m.generatedImage || m.attachment || m.generatedVideo,
        );
        if (lastMediaMsg) {
          attachmentToSend =
            lastMediaMsg.generatedImage ||
            lastMediaMsg.attachment ||
            lastMediaMsg.generatedVideo ||
            null;
        }
      }
      ...
      if (attachmentToSend) {
        ...
        // Add the image to attachments if not already present
        if (!args.attachments.includes(attachmentToSend)) {
          args.attachments.push(attachmentToSend);
        }
      }
    }

    // --- SENTRY CONTROL (CLIENT-SIDE) ---
    if (name === "controlAlwaysOnVision") {
      const enabled = args.enabled !== false; // Default true if not specified
      const instruction = args.instruction;
      window.dispatchEvent(
        new CustomEvent("sentry-toggle", {
          detail: { mode: "visual", enabled, instruction },
        }),
      );
      return `Visual Sentry (Room Guard) ${enabled ? "ACTIVATED" : "DEACTIVATED"}.${instruction ? ` Instruction: ${instruction}` : ""}`;
    }

    if (name === "controlAlwaysOnAudio") {
      const enabled = args.enabled !== false;
      const instruction = args.instruction;
      window.dispatchEvent(
        new CustomEvent("sentry-toggle", {
          detail: { mode: "audio", enabled, instruction },
        }),
      );
      return `Audio Sentry (God Ear) ${enabled ? "ACTIVATED" : "DEACTIVATED"}.${instruction ? ` Target: ${instruction}` : ""}`;
    }
    ...
    else if (name === "batchAnalyzeAndOrganizeDirectory")
      endpoint = apiUrl("/api/fs/batch-organize");
    ...
    // C2 Tools: Now handled via HackingProvider
    // Graph Tools
    ...
    // Messaging and OSINT Tools: Now handled via CommunicationProvider and IntelligenceProvider
    ...
    // Twitter/X
    else if (name === "postTweet") endpoint = apiUrl("/api/twitter/post");
    else if (name === "readTwitterTimeline") {
      endpoint = apiUrl(`/api/twitter/timeline?count=${args.count || 10}`);
      method = "GET";
      body = undefined;
    } else if (name === "likeTwitterPost")
      endpoint = apiUrl("/api/twitter/like");
    else if (name === "replyToTweet") endpoint = apiUrl("/api/twitter/reply");
    else if (name === "retweetTwitterPost")
      endpoint = apiUrl("/api/twitter/retweet");
    else if (name === "quoteTwitterPost")
      endpoint = apiUrl("/api/twitter/quote");
    else if (name === "followTwitterUser")
      endpoint = apiUrl("/api/twitter/follow");
    else if (name === "unfollowTwitterUser")
      endpoint = apiUrl("/api/twitter/unfollow");
    else if (name === "sendTwitterDM") endpoint = apiUrl("/api/twitter/dm");
    else if (name === "searchTwitter") {
      endpoint = apiUrl(
        `/api/twitter/search?q=${encodeURIComponent(args.query)}&count=${
          args.count || 10
        }`,
      );
      method = "GET";
      body = undefined;
    } else if (name === "getTwitterTrending") {
      endpoint = apiUrl("/api/twitter/trending");
      method = "GET";
      body = undefined;
    }
    // Instagram
    else if (name === "postInstagram") endpoint = apiUrl("/api/instagram/post");
    else if (name === "readInstagramFeed") {
      endpoint = apiUrl(`/api/instagram/feed?count=${args.count || 10}`);
      method = "GET";
      body = undefined;
    } else if (name === "likeInstagramPost")
      endpoint = apiUrl("/api/instagram/like");
    else if (name === "commentInstagramPost")
      endpoint = apiUrl("/api/instagram/comment");
    else if (name === "postInstagramStory")
      endpoint = apiUrl("/api/instagram/story");
    else if (name === "getInstagramStories") {
      endpoint = apiUrl("/api/instagram/stories");
      method = "GET";
      body = undefined;
    } else if (name === "followInstagramUser")
      endpoint = apiUrl("/api/instagram/follow");
    else if (name === "unfollowInstagramUser")
      endpoint = apiUrl("/api/instagram/unfollow");
    else if (name === "sendInstagramDM") endpoint = apiUrl("/api/instagram/dm");
    else if (name === "readInstagramDMs") {
      endpoint = apiUrl("/api/instagram/dms");
      method = "GET";
      body = undefined;
    } else if (name === "exploreInstagramContent") {
      endpoint = apiUrl("/api/instagram/explore");
      method = "GET";
      body = undefined;
    } else if (name === "getInstagramStats") {
      endpoint = apiUrl(`/api/instagram/stats?username=${args.username || ""}`);
      method = "GET";
      body = undefined;
    }
    // LinkedIn
    else if (name === "postLinkedIn") endpoint = apiUrl("/api/linkedin/post");
    else if (name === "readLinkedInFeed") {
      endpoint = apiUrl(`/api/linkedin/feed?count=${args.count || 10}`);
      method = "GET";
      body = undefined;
    } else if (name === "likeLinkedInPost")
      endpoint = apiUrl("/api/linkedin/like");
    else if (name === "commentLinkedInPost")
      endpoint = apiUrl("/api/linkedin/comment");
    else if (name === "sendLinkedInConnection")
      endpoint = apiUrl("/api/linkedin/connect");
    else if (name === "searchLinkedInJobs") {
      endpoint = apiUrl(
        `/api/linkedin/jobs?keywords=${encodeURIComponent(
          args.keywords,
        )}&location=${encodeURIComponent(args.location || "")}`,
      );
      method = "GET";
      body = undefined;
    } else if (name === "sendLinkedInMessage")
      endpoint = apiUrl("/api/linkedin/message");
    else if (name === "readLinkedInMessages") {
      endpoint = apiUrl("/api/linkedin/messages");
      method = "GET";
      body = undefined;
    } else if (name === "viewLinkedInProfile") {
      endpoint = apiUrl(
        `/api/linkedin/profile?profileUrl=${encodeURIComponent(
          args.profileUrl,
        )}`,
      );
      method = "GET";
    ...
    // Discord
    else if (name === "sendDiscordMessage")
      endpoint = apiUrl("/api/discord/send");
    else if (name === "readDiscordMessages") {
      endpoint = apiUrl(
        `/api/discord/messages?channelId=${args.channelId}&count=${
          args.count || 20
        }`,
      );
      method = "GET";
      body = undefined;
    } else if (name === "listDiscordServers") {
      endpoint = apiUrl("/api/discord/servers");
      method = "GET";
      body = undefined;
    }
    // YouTube
      ...
      endpoint = apiUrl(
        `/api/youtube/search?query=${encodeURIComponent(args.query)}&count=${
          args.count || 10
        }`,
      );
      method = "GET";
      body = undefined;
    } else if (name === "getYouTubeSubscriptions") {
      endpoint = apiUrl(`/api/youtube/subscriptions?count=${args.count || 20}`);
      method = "GET";
      body = undefined;
    } else if (name === "likeYouTubeVideo")
      endpoint = apiUrl("/api/youtube/like");
    else if (name === "commentYouTubeVideo")
      endpoint = apiUrl("/api/youtube/comment");
    // Social Skills (Self-Learning)
    else if (name === "startSocialSkillRecording")
      endpoint = apiUrl("/api/social-skills/record/start");
    else if (name === "stopSocialSkillRecording")
      endpoint = apiUrl("/api/social-skills/record/stop");
    else if (name === "listSocialSkills") {
      endpoint = apiUrl("/api/social-skills/list");
      method = "GET";
      body = undefined;
    } else if (name === "deleteSocialSkill") {
      endpoint = apiUrl(`/api/social-skills/${args.skillName}`);
      method = "DELETE";
      body = undefined;
    }

    // Inject Luca Link Credentials for Telegram (JIT Support)
    ...
    else if (name === "connectToMCPServer") {
      endpoint = apiUrl("/api/mcp/connect");
      // Auto-generate name if not provided, to satisfy backend requirement
      if (!args.name) {
        args.name = `mcp_server_${Date.now()}`;
      }
      body = {
        name: args.name,
        url: args.url,
        type: "sse", // Default to SSE for now as per tool def
    ...
    // --- SPECIAL: UI-TARS VISION INTEGRATION ---
      ...
      try {
        ...
        // 3. Agentic Vision Analysis
        const prediction = await agenticVisionService.analyze(
          screenshotBase64,
          instruction,
          undefined,
          args.model,
        ...
        return `[AGENTIC VISION] Analysis Complete.\nPrediction: ${prediction}`;
      } catch (e: any) {
        return `[AGENTIC] Error: ${e.message}`;
      }
    }
    ...
    // --- LUCA OPS: LOCKDOWN (IPC Priority) ---
    if (name === "initiateLockdown") {
      try {
        ...
        if (window.electron?.ipcRenderer) {
          ...
          const result =
            await window.electron.ipcRenderer.invoke("initiate-lockdown");
          if (result.success) {
            return `🔴 LOCKDOWN INITIATED\n${result.message}\n\nLucaOS Protocol: ACTIVE`;
          } else {
    ...
    try {
      ...
      // --- SAFETY VALVE BYPASS ---
      if (args?._lucaBypass) {
        console.log(`[DISPATCH] 🛡️ Applying Safety Valve Bypass for ${name}`);
        headers["X-LUCA-BYPASS"] = "true";
        // Clean up internal flag
        delete args._lucaBypass;
      }
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        // --- AUTOMATIC NETWORK TOPOLOGY CASTING ---
        if (name === "generateNetworkMap" && context.setVisualData) {
          context.setVisualData({
            type: "NETWORK_MAP",
        ...
        // --- AUTOMATIC IOT RADAR CASTING ---
        if (name === "scanLocalDevices" && context.setVisualData) {
          context.setVisualData({
            type: "IOT_RADAR",
            topic: "IOT_DISCOVERY",
            data: data,
        ...
        // --- AUTOMATIC IOT DEBUG CASTING ---
        if (name === "executeDeviceCommand" && context.setVisualData) {
          context.setVisualData({
            type: "IOT_LOG",
            topic: "IOT_EXECUTION",
            data: {
              ...data,
              log: args, // Show the arguments (payload) used
            },
          });
        }
        if (
        ...
        if (
          ...
          context.setCinemaUrl(
            ...
            sourceType,
            args?.title ||
              (isDrmPlatform ? "Streaming (DRM)" : "Now Streaming"),
          );
        }

        // --- SPECIAL HANDLING FOR PYTHON SANDBOX IMAGES ---
        if (
          ...
          // Append the image data sequentially to the stdout result
          // so useToolOrchestrator logs it in the TacticalStream
          let combinedResult = data.result || "";
