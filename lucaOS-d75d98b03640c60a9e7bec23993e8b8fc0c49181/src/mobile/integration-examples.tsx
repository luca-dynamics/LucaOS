...
/**
 * Example 1: Pass Navigation State to LucaService
...
export function setupGlobalErrorHandling() {
  ...
  window.addEventListener("error", (event) => {
    console.error("[Luca] Global error:", event.error);
  });
}
...
/**
 * Example 5: Listen for Luca Link Desktop Phoenix Response
 */
export function setupDesktopPhoenixHandler() {
  if (typeof (globalThis as any).lucaLinkManager !== "undefined") {
    const lucaLinkManager = (globalThis as any).lucaLinkManager;

    lucaLinkManager.on("command:received", async (event: any) => {
      if (event.data.message.payload.command === "phoenix:recovery") {
        const suggestions = event.data.message.payload.args;

        console.log(
          "[Luca] Received fix suggestions from desktop:",
          suggestions,
        );
