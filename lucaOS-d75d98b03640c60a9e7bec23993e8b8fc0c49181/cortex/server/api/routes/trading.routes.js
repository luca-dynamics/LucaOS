 ...
 * Central tool executor. Resolves exchange if missing.
 */
router.post('/execute', async (req, res) => {
  let { exchange, symbol, action, quantity, leverage } = req.body;
  
  if (!exchange) {
    const connected = exchangeManager.getConnectedExchanges();
    if (connected.length === 0) {
      return res.status(400).json({ success: false, error: 'No exchanges connected. Use Exchange Settings to add one.' });
    }
    exchange = connected[0].id; // Default to first connected
  }

  // Redirect to existing exchange order logic
  try {
    const response = await fetch(`http://localhost:3002/api/trading/exchange/${exchange}/order`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ symbol, action, quantity, leverage })
    });
 ...
 * Central position tracker. Resolves exchange if missing.
 */
...
// Storage paths (User Documents)
const STRATEGIES_DIR = path.join(LUCA_USER_DIR, 'strategies');
const TRADES_DIR = path.join(LUCA_USER_DIR, 'trades');

// Ensure storage directories exist
[STRATEGIES_DIR, TRADES_DIR].forEach(dir => {
