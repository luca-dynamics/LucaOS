...
router.get('/list', (req, res) => {
    try {
        ...
        // Get built-in and auto-discovered servers
        const builtInServers = mcpClientManager.getBuiltInServers ? 
            mcpClientManager.getBuiltInServers() : [];
        const autoDiscoveredServers = mcpClientManager.getAutoDiscoveredServers ?
            mcpClientManager.getAutoDiscoveredServers() : [];
        
        // Merge user servers with built-in and auto-discovered
        const userServers = settings.mcp?.servers || [];
        const allServers = [...builtInServers, ...autoDiscoveredServers, ...userServers];
