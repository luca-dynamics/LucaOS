...
class GoogleService {
    ...
    /**
     * Google workspace is checked via the accounts domain marker
     */
    hasChromeProfile() {
        const chromePath = paths.CHROME_PROFILES_DIR;
        if (!fs.existsSync(chromePath)) return false;
        
        const markers = [
            path.join(chromePath, 'IndexedDB', 'https_accounts.google.com_0.indexeddb.leveldb'),
            path.join(chromePath, 'Local Storage', 'leveldb')
        ];

        return fs.existsSync(markers[0]);
    }
