...
class GoalScheduler {
     ...
     * Called by God Mode Toggle
     */
    async startAutonomy(settings = null) {
        console.log('[GOAL_SCHEDULER] Enabling Autonomy (God Mode)...');
        
        if (settings && settings.brain) {
            this.modelConfig = {
                provider: settings.brain.provider || 'google',
                model: settings.brain.model || 'gemini-2.0-flash'
            };
            console.log(`[GOAL_SCHEDULER] Using Model Config: ${this.modelConfig.provider}/${this.modelConfig.model}`);
        }

        this.isAutonomyEnabled = true;
        this.initialize(); // Ensure everything is running
    }

    stopAutonomy() {
        console.log('[GOAL_SCHEDULER] Disabling Autonomy (God Mode)...');
        this.isAutonomyEnabled = false;
        // Stop all jobs immediately
        for (const job of this.jobs.values()) {
            job.stop();
        }
        this.jobs.clear();
        this.isInitialized = false; // Force re-init on start
    }
    ...
    getStatus() {
        return {
            title: 'God Mode',
            description: 'Autonomous Background Operations',
            initialized: this.isInitialized,
            enabled: this.isAutonomyEnabled || false,
